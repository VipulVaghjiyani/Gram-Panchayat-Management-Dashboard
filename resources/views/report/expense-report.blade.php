...
@section('content')
            ...
            <form id="filter_form">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-2 col-md-4 col-12 mb-4 mb-lg-0">
                        <div class="form-floating form-floating-outline">
                            <select class="form-select select2" id="expenseMemberId" name="expenseMemberId"
                                data-placeholder="Select Expense Member" aria-label="Default select example"
                                data-allow-clear="true">
                                <option value="" selected>Select Expense Member</option>
                                @foreach ($expense_members as $expense_member)
                                    <option value="{{ $expense_member->id }}" @if ($request->expenseMemberId == $expense_member->id) selected @endif>
                                        {{ $expense_member->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-lg-2 col-md-4 col-12 mb-4 mb-md-0">
                        <div class="form-floating form-floating-outline">
                            <select class="form-select select2" id="expenseCategoryId" name="expenseCategoryId"
                                data-placeholder="Select Expense Category" aria-label="Default select example"
                                data-allow-clear="true">
                                <option value="" selected>Select Expense Category</option>
                                @foreach ($expense_categories as $expense_category)
                                    <option value="{{ $expense_category->id }}"
                                        @if ($request->expenseCategoryId == $expense_category->id) selected @endif>{{ $expense_category->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-lg-2 col-md-4 col-12 mb-4 mb-md-0">
                        <div class="form-floating form-floating-outline">
                            <select class="form-select select2" id="accountId" name="accountId"
                                data-placeholder="Select Account" aria-label="Default select example"
                                data-allow-clear="true">
                                <option value="" selected>Select Account</option>
                                @foreach ($accounts as $account)
                                    <option value="{{ $account->id }}"
                                        @if ($request->accountId == $account->id) selected @endif>{{ $account->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
            <h5 class="card-title m-0 me-2 text-secondary">Expense Report</h5>
        </div>
        @if (session('message'))
        ...
        <div class="card-body">
            <div class="card-datatable table-responsive pt-0">
                ...
                <table class="datatables-basic table table-bordered" id="expense_table">
                    <!-- Overlay HTML -->
                    <div id="overlay">
                    ...
                    <tbody>
                        @foreach ($allData as $key => $item)
                            <tr>
                                ...
                                {{-- <td>{{ $item->expense_member_id ? $item->expenseMember->name : '' }}</td> --}}
                                <td>{{ $item->expense_member ?? '' }}</td>
                                {{-- <td>{{ $item->account_id ? $item->account->name : '' }}</td> --}}
                                <td>{{ $item->account ?? '' }}</td>
                                {{-- <td>{{ $item->expense_category_id ? $item->expenseCatgory->name : '' }}</td> --}}
                                <td>{{ $item->expense_category ?? '' }}</td>
                                <td>{{ $item->date ? \DateTime::createFromFormat('Y-m-d', $item->date)->format('d/m/Y') : '' }}
                                </td>
                                <td>{{ $item->amount }}</td>
                                ...
                                <td>{{ $item->transaction_date ? \DateTime::createFromFormat('Y-m-d', $item->transaction_date)->format('d/m/Y') : '' }}</td>
...
@section('scripts')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var expenseMemberId = urlParams.get('expenseMemberId') ?? "";
            var expenseCategoryId = urlParams.get('expenseCategoryId') ?? "";
            ...
            $('.page-item a.page-link').each(function(key, value) {
                ...
                let expenseMemberId = urlParams.get('expenseMemberId') ?? "";
                let expenseCategoryId = urlParams.get('expenseCategoryId') ?? "";
                ...
                if (page || pageLength) {
                    $(this).attr('href', baseUrl + '?' + page + '&page_length=' + pageLength + '&from=' +
                        from + '&to=' + to + '&expenseMemberId=' + expenseMemberId + '&expenseCategoryId=' +
                        expenseCategoryId + '&paymentType=' + paymentType + '&accountId=' + accountId);
                } else {
            ...
            $('#reset').click(function() {
                $('#from').val('');
                $('#to').val('');
                $('#expenseMemberId').val('').trigger('change');
            ...
            function fill_datatable(from = '', to = '') {
                ...
                dataTable = $('#expense_table').DataTable({
                    fixedHeader: {
                        header: true
                    },
                    "autoWidth": true,
                    ...
                    responsive: {
                        details: {
                            display: $.fn.dataTable.Responsive.display.modal({
                                ...
                                header: function(row) {
                                    var data = row.data();
                                    return 'Details of ' + data[3] + ' - ' + data[5];
                                }
                            }),
                            type: 'column',
