...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
            <h5 class="card-title m-0 me-2 text-secondary">Expense Listing</h5>
            @php
                $chk = \App\Models\Permission::checkCRUDPermissionToUser('Expense', 'create');
                if ($chk) {
                    echo '<a href="' .
                        route('expense.create') .
                        '" class="btn btn-primary"><i class="mdi mdi-plus me-sm-1"></i><span
                    class="d-none d-sm-inline-block">Create Expense Record</span></a>';
                }
            @endphp
            {{-- <a href="{{ route('expense.create') }}" class="btn btn-primary"><i class="mdi mdi-plus me-sm-1"></i><span
                    class="d-none d-sm-inline-block">Create Expense Record</span></a> --}}
        </div>
        @if (session('message'))
                        ...
                        @foreach ($allData as $key => $item)
                            <tr>
                                ...
                                <td>
                                    @php
                                        $html = '';
                                        $readCheck = \App\Models\Permission::checkCRUDPermissionToUser('Expense', 'read',);
                                        $updateCheck = \App\Models\Permission::checkCRUDPermissionToUser('Expense', 'update',);
                                        $isSuperAdmin = \App\Models\Permission::isSuperAdmin();
                                        if ($readCheck) {
                                            $html .= '<li><a class="dropdown-item dropdown-trigger-17500btn waves-effect" href="expense/' . $item->id . '">View</a></li>';
                                        }
                                        if ($updateCheck) {
                                            $html .= '<li><a class="dropdown-item dropdown-trigger-17500btn waves-effect" href="expense/' . $item->id . '/edit">Edit</a></li>';
                                        }
                                        if (!$isSuperAdmin && !$updateCheck && !$readCheck) {
                                            $html = '';
                                        }
                                        if ($isSuperAdmin) {
                                            $html .= '<li><a class="dropdown-item dropdown-trigger-17500btn waves-effect" href="javascript:void(0)" onclick="deleteExpense(' . $item->id . ')">Delete</a></li>';
                                        }
                                    @endphp
                                    <div class="row">
                                        <div class="col s12">
            ...
            $('.page-item a.page-link').each(function(key, value) {
                ...
                if (page || pageLength) {
                    $(this).attr('href', baseUrl + '?' + page + '&page_length=' + pageLength + '&from=' +
                        from + '&to=' + to + '&memberId=' + memberId + '&expenseCategoryId=' +
                        expenseCategoryId + '&paymentType=' + paymentType);
                } else {
                    ...
                    responsive: {
                        details: {
                            display: $.fn.dataTable.Responsive.display.modal({
                                ...
                                header: function(row) {
                                    var data = row.data();
                                    return 'Details of ' + data[4] + ' and ' + data[6];
                                }
                            }),
                            type: 'column',
